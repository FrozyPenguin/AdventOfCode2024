...
const patternsCache = new Map<string, boolean>();

function addToCache(pattern: string, value: boolean) {
  patternsCache.set(pattern, value);
  return value;
}

function isPatternPossible(pattern: string, components: string[]): boolean {
  if (patternsCache.has(pattern)) return patternsCache.get(pattern)!;
  if (pattern === "") return addToCache(pattern, true);
  for (const component of components) {
    if (!pattern.startsWith(component)) continue;
    const isPossible = isPatternPossible(
    ...
    return addToCache(pattern, true);
  }
  return addToCache(pattern, false);
}

const possiblePatterns = patterns.filter((pattern) =>
  isPatternPossible(pattern, towels)
);
