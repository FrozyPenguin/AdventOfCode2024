...
for (let y = 0; y < parsedInput.length; y++) {
  ...
  for (let x = 0; x < line.length; x++) {
    ...
    const possibleTrails: Array<Point[]> = getTrails(startPoint, parsedInput);
...
function getTrails(startPoint: Point, grid: GRID) {
  ...
  for (const side of sides) {
    ...
    const trailsFromPoint = getTrails(side, grid);
    for (const trail of trailsFromPoint) {
      if (trails.some((path) => path.at(-1)?.isSame(trail.at(-1)!))) continue;
