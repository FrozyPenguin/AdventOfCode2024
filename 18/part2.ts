...
const corruptedRamCoordinates: Set<string> = new Set(input.split(/[\r\n]+/g));

const startPoint = new Point(0, 0);
const endPoint = new Point(size - 1, size - 1);

function isEndReachable(currentCorruptedRamCoordinates: Set<string>): boolean {
  ...
  while (queue.length) {
    ...
    const moves = getPossibleNodesToMove(
      node,
      currentCorruptedRamCoordinates,
      size
...
for (let i = sample; i < corruptedRamCoordinates.size; i += steps) {
  const isReachable = isEndReachable(
    new Set([...corruptedRamCoordinates].slice(0, i + 1))
  );

  if (!isReachable) {
    if (steps === 1) {
      console.log([...corruptedRamCoordinates][i]);
      break;
    }
